...
class Worker
{
    ...
    protected static function init()
    {
        ...
        if (empty(self::$pidFile)) {
			...
			self::$pidFile = ENTRY_PATH . "workerman.pid";
        }

        // Log file.
        if (empty(self::$logFile)) {
            //self::$logFile = __DIR__ . '/../workerman.log';
			$logDir = SOLUTION_LOG_PATH . 'workerman/';
			mkdir ( $logDir, 0777,true);
			self::$logFile =  $logDir. APP_PREFIX . '.log';
        }
    ...
    public function listen()
    {
        ...
        if (!$this->_mainSocket) {
			echo '[error][Worker.php]stream_socket_server error ' . $local_socket . ' errno='.$errno . ' errmsg=' . $errmsg .PHP_EOL;
			echo '[php.ini]disable_functions=' .ini_get('disable_functions') .PHP_EOL;
